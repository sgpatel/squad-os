...
public class FraudDetectionApp {
    ...
    private void runScenario(SquadContext ctx,
        ...
        // ── @SquadTool: RiskAnalyst queries data ──────────────────
        ...
        RiskAssessment risk = ctx.submitTo(AgentRole.ANALYST,
            ...
            txContext + "\n" +
            "Provide a complete risk assessment with riskScore (0.0-1.0), " +
        ...
        // ── @SecureAgent: set compliance identity ─────────────────
        ...
        String voteReason = txContext + " | riskScore=" + riskScore;
        if (riskScore < 0.4) {
            collector.submit(Vote.approve("Risk score low, transaction pattern normal", 1.0)
                .withVoter("RiskAnalyst"), "RiskAnalyst");
            collector.submit(Vote.approve("Behaviour consistent with profile", 1.0)
                .withVoter("BehaviourAgent"), "BehaviourAgent");
            collector.submit(Vote.approve("Passes AML/KYC checks", 1.0)
                .withVoter("ComplianceAgent"), "ComplianceAgent");
        } else {
            collector.submit(Vote.reject("Risk score elevated: " + riskScore, 1.0)
                .withVoter("RiskAnalyst"), "RiskAnalyst");
            collector.submit(Vote.reject("Behaviour anomaly detected", 1.0)
                .withVoter("BehaviourAgent"), "BehaviourAgent");
            collector.submit(Vote.reject("AML threshold exceeded", 1.0)
                .withVoter("ComplianceAgent"), "ComplianceAgent");
        }
        ...
        try {
            ...
            voteResult = new VoteResult(io.squados.vote.VoteResult.Outcome.TIMEOUT, new java.util.ArrayList<>(), "fraud-verdict");
        }
        ...
        System.out.println();
        if (riskScore < 0.3) {
            System.out.println("[AutoApproval] riskScore " + riskScore +
                " < 0.3 — AUTO-APPROVED");
        } else if (riskScore > 0.7) {
            System.out.println("[AwaitApproval] riskScore " + riskScore +
                " > 0.7 — ESCALATING to senior-fraud-analyst");
        } else {
            System.out.println("[Decision] riskScore " + riskScore +
                " — APPROVE with monitoring flag");
        }
        ...
        FeedbackExample.Label feedbackLabel = riskScore < 0.5
            ? FeedbackExample.Label.GOOD : FeedbackExample.Label.BAD;
        FeedbackExample example = new FeedbackExample(
            "fraud-risk-assessment", txContext,
            "riskScore=" + riskScore + " recommendation=" + risk.recommendation,
            feedbackLabel, "Processed correctly");
        ...
        System.out.println("  Feedback:  " + feedbackStore.count("fraud-risk-assessment") +
            " examples stored for @Improve");
    }

    private void printBanner() {
        System.out.println();
        System.out.println("\uD83D\uDEE1\uFE0F  FRAUD DETECTION SQUAD \uD83D\uDEE1\uFE0F");
        System.out.println("Powered by SquadOS v3.1 — 15 annotations, 340 tests");
